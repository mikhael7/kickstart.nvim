...
local function git_log(path, range, callback)
  vim.system({
    ...
    '--oneline',
    range,
  }, { text = true }, function(res) callback(parse_commits(res.code == 0 and res.stdout or '')) end)
end

-- Returns true if a single commit line has a conventional breaking marker
-- Matches 'type!:' or 'type(scope)!:'
local function is_breaking_commit(c) return c:match '%x+ %w+!:' or c:match '%x+ %w+%b()!:' end

-- Returns true if any commit line contains a breaking change marker
local function has_breaking_commit(commits) return vim.iter(commits):any(is_breaking_commit) end

-- Collect only the breaking commit lines from a list
local function filter_breaking(commits) return vim.iter(commits):filter(is_breaking_commit):totable() end
...
local function check_updates()
  ...
  local function finish_one(result)
    vim.schedule(function()
      ...
      if result then
        if result.updates ~= nil then state.updates[result.name] = result.updates end
        if result.breaking then state.breaking[result.name] = true end
        if result.unreleased_breaking then state.unreleased_breaking[result.name] = result.unreleased_breaking end
...
local function build_content()
  ...
  local function add(text, hl)
    local lnum = #lines
    lines[#lines + 1] = text
    if hl then table.insert(hls, { lnum, 0, #text, hl }) end
  end

  local function add_hl(lnum, col_start, col_end, hl) table.insert(hls, { lnum, col_start, col_end, hl }) end
  ...
  local function render_plugin(p, icon, hl_group)
    ...
    if latest then
      ...
      if latest_display ~= ver_display then ver_display = ver_display .. ' → ' .. latest_display end
    end
    ...
    local unreleased_str = unreleased and #unreleased > 0 and string.format('  ⚠ %d breaking unreleased', #unreleased) or ''
    ...
    if update_count > 0 then
      ...
      add_hl(lnum_cur, update_start, update_start + #update_str, state.breaking[name] and 'PackUiBreaking' or 'PackUiUpdateAvailable')
    end
...
local function jump_plugin(direction)
  ...
  if direction > 0 then
    ...
    -- Wrap around
    if #plines > 0 then api.nvim_win_set_cursor(state.winid, { plines[1], 0 }) end
  else
    for i = #plines, 1, -1 do
    ...
    -- Wrap around
    if #plines > 0 then api.nvim_win_set_cursor(state.winid, { plines[#plines], 0 }) end
  end
end
...
local function setup_keymaps()
  ...
  vim.keymap.set('n', 'X', function()
    local to_clean = vim.iter(vim.pack.get(nil, { info = false })):filter(function(x) return not x.active end):map(function(x) return x.spec.name end):totable()
  ...
  vim.keymap.set('n', '<CR>', function()
    ...
    if name then
      ...
      if state.plugin_lines[name] then api.nvim_win_set_cursor(state.winid, { state.plugin_lines[name], 0 }) end
    end
  end, opts)
